...
def GlaisherBijection (n : ℕ) : odds n ≃ distincts n where
  toFun := fun ⟨P, P_odd⟩ ↦ ⟨FromOdd P, InDist P P_odd⟩
  invFun := fun ⟨Q, Q_dist⟩ ↦ ⟨FromDist Q Q_dist, InOdd Q Q_dist⟩
  left_inv := by intro ⟨P, P_odd⟩; simp only [Subtype.mk.injEq]; exact LeftInv P P_odd
  right_inv := by intro ⟨Q, Q_dist⟩; simp only [Subtype.mk.injEq]; exact RightInv Q Q_dist

theorem EulerIdenity (n : ℕ) : (odds n).card = (distincts n).card :=
  Finset.card_eq_of_equiv (GlaisherBijection n)

theorem EulerPartsCount
